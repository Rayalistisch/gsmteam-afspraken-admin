...
async function sendMailgun({
  apiKey, domain, region, from, to, subject, html, attachments,
}: {
...
export async function POST(req: Request) {
  try {
    ...
    if (data.status !== "awaiting_approval") {
      return NextResponse.json({ error: "Herinneringsmail kan alleen verstuurd worden bij status 'wacht op klant'" }, { status: 400 });
    }
    ...
    try {
      ...
      const subject   = "Herinnering: offerte reparatie – GSM Team";
      ...
      // Mail verstuurd — sla tijdstip op
      ...
      if (updateError) {
        console.error("reminder_sent_at update error:", updateError.message, updateError.details);
      }

      return NextResponse.json({ ok: true, data, mail_sent: true, reminder_saved: !updateError });
    } catch (mailErr: any) {
      console.error("Offer reminder mail/pdf error:", mailErr);
      return NextResponse.json({ ok: true, data, mail_sent: false, mail_error: safe(mailErr?.message) });
    }
  } catch (err: any) {
    console.error("Offer reminder route error:", err);
